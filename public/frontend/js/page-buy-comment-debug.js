...
(function($){
	"use strict";
	var AVT_BUY_COMMENT = Backbone.View.extend({
		el : '#avt-page-buy-comment',

		formClassError : 'md-input-danger',

		events: {
			'submit #avt-form-buy-comment' : 'handleForm'
		},
		...
		handlePay: function( event ){
			var data = {
                formData: $('#avt-form-buy-comment', this.el).serialize()
            },
            self = this;
			altair_helpers.content_preloader_show();
			// Send to server handle.
            $.ajax({
			    url: AVTDATA.SITE_URL + '/validate-buy-comment',
			    type: "POST",
			    data: data,
			    success: function ( res ) {
			    	...
			    	if( false === dataResult.status ) {
			    		...
			    		window.location = location.href = AVTDATA.SITE_URL + '/buy-packet-comment';;
			    	}
			    }
			});
		}
