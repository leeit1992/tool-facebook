...
(function($){
	"use strict";
	var AVT_BUY_ADD_LIKE = Backbone.View.extend({
		el : '#avt-page-buy-add-like',

		formClassError : 'md-input-danger',

		events: {
			'submit #avt-form-buy-add-like' : 'handleForm'
		},
		...
		handlePay: function( event ){
			var data = {
                formData: $('#avt-form-buy-add-like', this.el).serialize()
            },
            self = this;
			altair_helpers.content_preloader_show();
			// Send to server handle.
            $.ajax({
			    url: AVTDATA.SITE_URL + '/validate-buy-add-like',
			    type: "POST",
			    data: data,
			    success: function ( res ) {
			    	...
			    	if( false === dataResult.status ) {
			    		...
			    		window.location = location.href = AVTDATA.SITE_URL + '/buy-add-like';;
			    	}
			    }
			});
		}
	});
	new AVT_BUY_ADD_LIKE;
