...
(function($){
    "use strict";
    var PAGE_UP_SHARE = Backbone.View.extend({
        el : '.pita-page-up-share',

        events: {
            'change #pita-share-number' : 'changePacket',
        ...
        changePacket : function(){
            var data = {
                id : $('#pita-share-number', this.el).val() },
                self = this;
            $(".pita-start-action").show();
            // Send to server handle.
            $.post(AVTDATA.SITE_URL + '/get-up-share', data, function(result) {
                var dataResult = JSON.parse( result );
                $(".avt-message-limit").text('');
                $(".pita-start-action").show();
                $(".pita-setinteval").val( dataResult.numberS );
                $(".avt_total_share").val( dataResult.total );
        ...
        startAction : function(){
            var self = this;
            var numberShare = $('.avt_share_number').val();
            var totalShare = $('.avt_total_share').val();
            
            if ( parseInt(numberShare) < parseInt(totalShare) ) {
                ...
                $(".avt-total-token-check").attr('data-total', numberShare );
                $(".avt-total-token-check").text( numberShare + ' Share ');
        ...
        ajaxAction: function( start = 0, limit = 1, totalToken = 0, timeAction = 2000){
            ...
            setTimeout(function(){
                $.post(AVTDATA.SITE_URL + '/ajax-tool-action', data, function(result){
                    ...
                    var numberTotal = $(".avt_total_share").val();
                    $(".avt_total_share").val( numberTotal - 1 );
