...
(function($){
    "use strict";
    var ATL_ADD_SHARE = Backbone.View.extend({
        el : '#avt-page-share',

        formClassError : 'md-input-danger',

        events: {
            'submit #avt-form-share' : 'handleForm',
            'click .atl-manage-share-delete-js' : 'removeShare',
        ...
        handleShare: function( event ){
            var data = {
                formData: $('#avt-form-share', this.el).serialize()
            },
            self = this;
            altair_helpers.content_preloader_show();
            // Send to server handle.
            $.ajax({
                url: AVTDATA.SITE_URL + '/validate-share',
                type: "POST",
                data: data,
                success: function ( res ) {
                    ...
                    if( false === dataResult.status ) {
                        ...
                        window.location = location.href = AVTDATA.SITE_URL + '/edit-share/' + dataResult.id;
                    }
                }
            });
        },
        ...
        removeShare: function(e){
            ...
            UIkit.modal.confirm('Are you sure?', function(){ 
                var data = { id: dataID };
                $.post(AVTDATA.SITE_URL + '/delete-share', data, function(result) {
