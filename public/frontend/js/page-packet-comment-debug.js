...
(function($){
    "use strict";
    var ATL_PACKET_COMMENT = Backbone.View.extend({
        el : '#avt-page-packet-comment',

        formClassError : 'md-input-danger',

        events: {
            'submit #avt-form-packet-comment' : 'handleForm',
            'click .atl-manage-packet-comment-delete-js' : 'removePacketComment',
        ...
        handleForm: function( event ){
            ...
            if( 0 === error.length ){
                this.handlePacketComment( event );
                return false;
            }else{
                return false;
            }
        ...
        handlePacketComment: function( event ){
            var data = {
                formData: $('#avt-form-packet-comment', this.el).serialize()
            },
            self = this;
            altair_helpers.content_preloader_show();
            // Send to server handle.
            $.ajax({
                url: AVTDATA.SITE_URL + '/validate-packet-comment',
                type: "POST",
                data: data,
                success: function ( res ) {
                    ...
                    if( false === dataResult.status ) {
                        ...
                        window.location = location.href = AVTDATA.SITE_URL + '/edit-packet-comment/' + dataResult.id;
                    }
                }
            });
        },
         ...
         * @return void
         */
        removePacketComment: function(e){
            var dataID = $( e.currentTarget ).attr('data-id');
            UIkit.modal.confirm('Are you sure?', function(){ 
                var data = { id: dataID };
                $.post(AVTDATA.SITE_URL + '/delete-packet-comment', data, function(result) {
            ...
            if( 'delete' == action ) {
                ...
                $.post(AVTDATA.SITE_URL + '/delete-packet-comment', data, function(result) {
                    ...
                    $.each(argsID,function(i, el){
                        $(".atl-packet-comment-item-" + el).remove();
                    })
                    altair_helpers.content_preloader_hide();
