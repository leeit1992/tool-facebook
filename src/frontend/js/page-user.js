...
(function($){
	"use strict";
	var AVT_USER = Backbone.View.extend({
		el : '#avt-page-handle-user',

		formClassError : 'md-input-danger',

		events: {
			'submit #avt-form-user' : 'handleForm',
		...
		handleUser: function( event ){
			var data = {
                formData: $('#avt-form-user', this.el).serialize()
            },
            self = this;
			altair_helpers.content_preloader_show();
			// Send to server handle.
            $.ajax({
			    url: AVTDATA.SITE_URL + '/validate-user',
			    type: "POST",
			    data: data,
			    success: function ( res ) {
			    	...
			    	if( false === dataResult.status ) {
			    		...
			    		window.location = location.href = AVTDATA.SITE_URL + '/edit-user/' + dataResult.id;
			    	}
			    }
			});
		},
		...
		handleFilterByRole: function(e){
			...
			var data = {
          	...
          	// Send to server handle.
    		$.get(AVTDATA.SITE_URL + '/ajax-manage-user', data, function(result) {
		...
		removeUser: function(e){
			...
			UIkit.modal.confirm('Are you sure?', function(){ 
				var data = { id: dataID	};
				$.post(AVTDATA.SITE_URL + '/delete-user', data, function(result) {
